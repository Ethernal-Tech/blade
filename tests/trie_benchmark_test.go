...
const (
	...
	dbPathLevelDb  = "/tmp/blockchain-disk-usage-test/leveldb"
...
func applyTransactions(transactions []*types.Transaction,
	...
	for _, m := range transactions {
		...
		m.SetNonce(m.Nonce() + uint64(numberOfTransactions))
		m.SetValue(big.NewInt(1))
	}
	...
	for j := 0; j < count; j++ {
		...
		txData =
			types.NewDynamicFeeTx(
				types.WithGasFeeCap(big.NewInt(0x01)),
				types.WithGasTipCap(big.NewInt(0x01)),
				types.WithAccessList(nil),
				types.WithFrom(from),
				types.WithTo(&incrementedAddress),
				types.WithNonce(uint64(1)+uint64(j)),
				types.WithValue(value),
				types.WithGas(gasLimit),
				types.WithInput(nil),
			)
...
func buildStateWithHelperAccounts(numberOfTransactionsPerBlock uint64, db DBType) (state.State, state.Snapshot, types.Hash, error) {

	storage_object, err := initializeStorageObject(db)
	if err != nil {
		return nil, nil, types.ZeroHash, err
	}
...
func initializeStorageObject(db DBType) (itrie.Storage, error) {
	...
	if db == PebbleDB {
		...
		err := common.CreateDirSafe(dbPathLevelDb, 0755)
		if err != nil {
			return nil, err
		}
		storage_object, err = itrie.NewLevelDBStorage(filepath.Join(dbPathLevelDb, "trie"), hclog.NewNullLogger())
		if err != nil {
			fmt.Printf("Error creating storage object: %v\n", err)
			return nil, err
		}
